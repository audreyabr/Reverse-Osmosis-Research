...
empty_tank_dist = 25.635;  % cm, top of the tank to the top of the drainage square with some extra room
full_tank_dist = 20.223;  % cm  (CHANGE LATER?)
...
while run == 1
    ...
    if tank_state == 0
        ...
        % flush brine to maximum flush distance
        if tank_distance <= max_flush_distance
            % Drain max water in the tank
        ...
        if conductivity_list(end) > initial_conductivity + conductivity_buffer
            ...
            while conductivity_list(end) > initial_conductivity + conductivity_buffer       
                disp("Flushing after feeding stops")
